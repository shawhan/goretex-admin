...
(function($) {
	...
	Calendar.prototype._calculate_hour_minutes = function(data) {
		...
		var counts = {};
		for (var i = 0, j = eventTimeList.length; i < j; i++) {
		   if (counts[eventTimeList[i].start]) {
		      counts[eventTimeList[i].start]++;
		   }
		   else {
		      counts[eventTimeList[i].start] = 1;
		   } 
		}
		$.each(eventTimeList, function(k, e) {
			if(counts[e.start] > 1) {
				data.by_hour[e.index].sort = counts[e.start] -1;
			}
			counts[e.start]--;
		});
	};
